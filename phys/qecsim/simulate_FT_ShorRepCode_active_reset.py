...
from qecsim.FT_Shor_rep_code_generator import FTShorRepCodeGenerator
...
for distance in distance_vec:
    ...
    repc = FTShorRepCodeGenerator(num_stabilizer=num_stabilizer,
    ...
    for num_rounds in rounds_vec:
        ...
        for n in tqdm(range(num_iterations)):
            ...
            for i in range(num_rounds - 1):
                repc.generate_pre_stabilizer_round(stab_to_restart=range(num_stabilizer),plot=plot).apply_to(state)
                meas_anc_parity = np.array([state.classical[cb] for cb in repc.cbit_names[2::4]])
                to_reset = []
                stab_to_restart=np.nonzero(meas_anc_parity)[0]
                for i in stab_to_restart:
                    to_reset.append(repc.qubit_names[2+4 * i])
                    to_reset.append(repc.qubit_names[1 + 4 * i])
                repc.generate_active_reset(to_reset, plot=plot).apply_to(state)

                # while len(np.nonzero(meas_anc_parity)[0]):
                #     to_reset = []
                #     stab_to_restart=np.nonzero(meas_anc_parity)[0]
                #     for i in stab_to_restart:
                #         to_reset.append(repc.qubit_names[2+4 * i])
                #         to_reset.append(repc.qubit_names[1 + 4 * i])
                #     repc.generate_active_reset(to_reset, plot=plot).apply_to(state)
                #     repc.generate_pre_stabilizer_round(stab_to_restart=stab_to_restart, plot=plot).apply_to(state)
                #     for i in stab_to_restart:
                #         meas_anc_parity[i] = np.array([state.classical[repc.cbit_names[2 + i * 4]]])[0]
                stabilizer.apply_to(state)
                meas = np.array([state.classical[cb] for cb in repc.cbit_names[1::4] + repc.cbit_names[3::4]])
                syndromes.append(np.bitwise_xor(meas[0:num_stabilizer], meas[num_stabilizer::]))
                # apply active reset
                to_reset = []
                for q1, q2, cb1, cb2 in zip(repc.qubit_names[1::4], repc.qubit_names[3::4], repc.cbit_names[1::4], repc.cbit_names[3::4]):
                    if state.classical[cb1] == 1:
                        to_reset.append(q1)
                    if state.classical[cb2] == 1:
                        to_reset.append(q2)
                to_reset.append(repc.qubit_names[2::4])
                repc.generate_active_reset(to_reset, plot=plot).apply_to(state)
            meas_anc_parity = np.array([state.classical[cb] for cb in repc.cbit_names[2::4]])
            for i in range(len(np.nonzero(meas_anc_parity)[0])):
                to_reset.append(repc.qubit_names[2 + 4 * np.nonzero(meas_anc_parity)[0][i]])
                to_reset.append(repc.qubit_names[1 + 4 * np.nonzero(meas_anc_parity)[0][i]])
            repc.generate_active_reset(to_reset, plot=plot).apply_to(state)
            repc.generate_stabilizer_round(final_round=True, plot=plot).apply_to(state)
            meas = np.array([state.classical[cb] for cb in repc.cbit_names[1::3] + repc.cbit_names[2::3]])
            syndromes.append(np.bitwise_xor(meas[0:num_stabilizer], meas[num_stabilizer::]))
            data_meas = np.array([state.classical[cb] for cb in repc.cbit_names[::3]])
...
sdh.log.info(f'automated reset tracking, {num_iterations} repetitions\n')
