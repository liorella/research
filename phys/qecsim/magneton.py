    ...
    with for_(i, 0, i < num_rounds, i+1):
        ...
        with switch_(boo):
            with case_(0):
                ...
                play("tick", "SCLK")
                wait(5, "MOSI") #5 clock cycles is 1 bit
                wait(5, "MOSI")
                wait(5, "MOSI")
                wait(5, "MOSI")
            with case_(1):
...
if use_simulator:
    job = qmm.simulate(config, prog, SimulationConfig(int(300*(num_rounds+1))))  # in clock cycles, 4 ns
    ...
    # trig = samples.con1.digital['10'] + 4.5
    ...
    ax.plot(trig[200:], label="FROMTRIG")
    ax.legend()
    ax.set_yticks([])
